...
export const UNITS: Record<string, UnitConfig> = {
  ...
  santimetr: { slug: "santimetr", label: "Santimetr", symbol: "cm", variants: ["sm", "santimeter", "cm"] },
  millimetr: { slug: "millimetr", label: "Millimetr", symbol: "mm", variants: ["millimeter", "mm"] },
  metr: { slug: "metr", label: "Metr", symbol: "m", variants: ["metir", "miter", "meter", "m"] },
  fut: { slug: "fut", label: "Fut", symbol: "ft", variants: ["foot", "ft"] },
  kilometr: { slug: "kilometr", label: "Kilometr", symbol: "km", variants: ["kilometer", "km"] },
  mil: { slug: "mil", label: "Mil", symbol: "mi", variants: ["mile", "mi"] },

  // Weight
  kg: { slug: "kg", label: "Kilogram", symbol: "kg", variants: ["kilo", "kilogramm", "kilogram", "kg"] },
  ...
  // Temperature
  selsiy: { slug: "selsiy", label: "Selsiy", symbol: "°C", variants: ["selsi", "celsius", "c"] },
  farengeyt: { slug: "farengeyt", label: "Farengeyt", symbol: "°F", variants: ["fahrenheit", "f"] },
  ...
  kub_metr: { slug: "kub-metr", label: "Kub metr", symbol: "m³", variants: ["m3", "kub metr", "kubmetr"] },
};

// Helper: unit mavjudligini tekshirish (dev safety)
export function assertUnitExists(unitSlug: string) {
  if (!Object.values(UNITS).some((u) => u.slug === unitSlug || u.slug === unitSlug.replace("_", "-"))) {
...
export function getAllConverterPages() {
  return (Object.entries(CATEGORIES) as Array<[CategorySlug, CategoryConfig]>).flatMap(([category, cfg]) =>
    cfg.pairs.map((p) => ({
      category,
      from: p.from,
      to: p.to,
